...
public:
	...
	void set_arg_value(ir_type* arg) override{
		...
		//TODO add ifdef flag here for preprocessor. Same for every other represent as string func call
		// arg->represent_as_string();
		if ( (passed_arguments.empty()) || (passed_arguments.back()->is_updated())){
			passed_arguments.push_back(arg);
		}else{
			if (passed_arguments.empty()){
				assert("Argument vector empty. Should not");
			}
...
public:
	...
	void print() override{
		...
		if (return_value != nullptr){
			...
			assert("Return value of exit_point is null!");
		}
		std::cout << std::endl;
		return;
	}
